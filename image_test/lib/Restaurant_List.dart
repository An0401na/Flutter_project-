import 'dart:convert';

import 'package:flutter/material.dart';

import 'Res.dart';
import 'Service_Res.dart';

class Restaurant_List extends StatefulWidget {
  Restaurant_List() : super();
  @override
  _Restaurant_ListState createState() => _Restaurant_ListState();
}

class _Restaurant_ListState extends State<Restaurant_List> {
  @override
  late List<Res> _res = [];
  late List<ResImageDir> _resImageDir = [];
  late bool _loading;

  void initState() {
    super.initState();
    _loading = true;
    Services_Res.getRests().then((Res1) {
      setState(() {
        _res = Res1;
        _loading = false;
      });
    });
    Services_Res.getRestImage().then((ResImage) {
      print("aaa");
      setState(() {
        _resImageDir = ResImage;
      });
    });
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text(_loading ? 'Loading ... ' 'Json Parsing' : '가게 목록'),
        ),
        body: Container(
          color: Colors.white,
          child: ListView.builder(
              itemCount: null == _res ? 0 : _res.length,
              itemBuilder: (context, index) {
                Res res = _res[index];
                return ListView.builder(
                    itemCount: null == _resImageDir ? 0 : _resImageDir.length,
                    itemBuilder: (context, index) {
                      ResImageDir resImageDir = _resImageDir[index];
                      return ListTile(
                        title: Row(
                          children: [Text(resImageDir.data.toString())],
                        ),
                        subtitle: Text(res.resName.toString()),
                      );
                    });
                // return ListTile(
                // title: Row(
                //   children: [
                //     Image.memory(
                //       base64Decode(res.image),
                //       width: 100,
                //       height: 100,
                //     ),
                //     Text(res.resName),
                //   ],
                // ),
                //   title: Text(res.resName),
                //   subtitle:
                //       Text('최소주문 금액 : ' + res.resMinOrderPrice.toString()),
                // );
              }),
        ));
  }
}
